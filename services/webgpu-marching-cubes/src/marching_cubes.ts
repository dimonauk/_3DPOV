...
export class MarchingCubes
{
    ...
    #activeVoxelOffsets: GPUBuffer;

    #atomicCounter: GPUBuffer;
    ...
    static async create(volume: Volume, device: GPUDevice)
    {
        ...
        mc.#activeVoxelOffsets = device.createBuffer({
            size: mc.#voxelActive.size,
            usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.COPY_SRC | GPUBufferUsage.STORAGE
        });

        mc.#atomicCounter = device.createBuffer({
            size: 4,
            usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.COPY_SRC | GPUBufferUsage.STORAGE
        });

        mc.#counterReadback = device.createBuffer({
            size: 4,
        ...
        mc.#markActiveBG = device.createBindGroup({
            ...
            entries: [
                ...
                },
                {
                    binding: 1,
                    resource: {
                        buffer: mc.#activeVoxelOffsets,
                    }
                },
                {
                    binding: 2,
                    resource: {
                        buffer: mc.#atomicCounter,
                    }
                }
            ]
        });
    ...
    private async computeActiveVoxels()
    {
        ...
        // Write timestamp queries to record perf if we've got support
        let passDesc: GPUComputePassDescriptor = {};
        if (this.#timestampQuerySupport) {
            passDesc = {
                timestampWrites: {
        ...
        commandEncoder.clearBuffer(this.#atomicCounter);

        let pass = commandEncoder.beginComputePass(passDesc);
        ...
        // Readback the atomic counter w/ total # of active voxels
        commandEncoder.copyBufferToBuffer(this.#atomicCounter, 0, this.#counterReadback, 0, 4);

        this.#device.queue.submit([commandEncoder.finish()]);
        await this.#device.queue.onSubmittedWorkDone();

        // Readback the atomic counter result
        await this.#counterReadback.mapAsync(GPUMapMode.READ);
        const nActive = new Uint32Array(this.#counterReadback.getMappedRange())[0];
    ...
    private async computeVertexOffsets(activeVoxels: MarchingCubesResult)
    {
        ...
        await this.#counterReadback.mapAsync(GPUMapMode.READ);
        const nVertices = new Uint32Array(this.#counterReadback.getMappedRange())[0];
        this.#counterReadback.unmap();
