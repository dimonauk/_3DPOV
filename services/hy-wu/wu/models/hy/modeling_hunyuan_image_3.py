...
from .cache_utils import CacheWithFreqsContainer
...
from .moe import HunyuanMoEEager
from .moe.base import HunyuanMLP
from .siglip2 import LightProjector, Siglip2VisionTransformer
from .system_prompt import get_system_prompt
from .tokenization_hunyuan_image_3 import CondImage, HunyuanImage3TokenizerFast, ImageInfo, ImageTensor
...
class HunyuanImage3DecoderLayer(nn.Module):
    def __init__(self, config: HunyuanImage3Config, layer_idx: int):
        ...
        if use_moe_layer:
            if config.moe_impl == "eager":
                self.mlp = HunyuanMoEEager(config, layer_idx=layer_idx)
            # elif config.moe_impl == "flashinfer":
            #     self.mlp = HunyuanMoEFlashInfer(config, layer_idx=layer_idx)
            # elif config.moe_impl == "torchtitan":
            #     self.mlp = HunyuanMoETorchTitan(config, layer_idx=layer_idx)
            else:
                raise ValueError(f"Unsupported moe implementation: {config.moe_impl}")
        else:
...
class HunyuanImage3ForCausalMM(HunyuanImage3PreTrainedModel, GenerationMixin):
    ...
    def instantiate_vae_image_tokens(
        ...
        if isinstance(images, torch.Tensor):
            ...
            for i, (image_i, t_i) in enumerate(zip(images, timesteps)):
                ...
                image_i_index = (
                    index[i : i + 1].masked_select(image_mask[i : i + 1].bool()).reshape(1, -1)
                )  # (1, img_seqlen)
    ...
    def ragged_final_layer(self, hidden_states, image_mask, timesteps, token_h, token_w, first_step=None):
        ...
        if isinstance(timesteps, torch.Tensor):
            ...
            image_output = (
                hidden_states.masked_select(image_mask.unsqueeze(-1).bool()).reshape(-1, n_embd).split(sections)
            )
            pred = []
    ...
    def forward(
        ...
        if input_ids is not None:
            hidden_states = self.model.get_wte(input_ids)  # (bsz, seqlen, n_embd)
        else:
        ...
        if images is not None:
            if self.config.cfg_distilled and input_ids is None:
                hidden_states = self.instantiate_vae_image_tokens(
                    hidden_states, timesteps, images, image_mask, guidance, timesteps_r
                )
            else:
        ...
        if cond_vae_images is not None:
            hidden_states = self.instantiate_vae_image_tokens(
                hidden_states, cond_timesteps, cond_vae_images, cond_vae_image_mask
            )

        if cond_vit_images is not None:
            hidden_states = self.instantiate_vit_image_tokens(
                hidden_states, cond_vit_images, cond_vit_image_mask, cond_vit_image_kwargs
            )
        ...
        if not self.use_taylor_cache:
            ...
            full_computation = (
                ...
                or (
                    ...
                    and cache_dic["current_step"] >= cache_dic["num_steps"] - cache_dic["tailing_enhance_steps"]
                )
            )
            ...
            if full_computation:
                ...
                if cache_dic["enable_first_enhance"] and (
                    cache_dic["current_step"] < (cache_dic["first_enhance_steps"] - 1)
                ):
                    pass
                else:
                    self.model.taylor_cache.derivatives_computation(
                        hidden_states,
                        distance=cache_dic["current_step"] - self.model.last_full_computation_step,
        ...
        if images is not None:
            ...
            diff_pred = self.ragged_final_layer(hidden_states, image_mask, timesteps, token_h, token_w, first_step)
        else:
            diff_pred = None
        # -- text tokens
        if input_ids is None or mode == "gen_image":
            ...
            hidden_states = self.model.get_ln_f(hidden_states)
    ...
    def _validate_and_batchify_image(image, name, check_batch_size=None):
        ...
        if check_batch_size is not None:
            assert (
                len(batch_image_list) == check_batch_size
    ...
    def vae_encode(self, image, cfg_factor=1, generator=None):
        ...
        with torch.autocast(
            device_type="cuda", dtype=self.vae_autocast_dtype, enabled=self.vae_autocast_dtype != torch.float32  # noqa
        ):
    ...
    def _encode_cond_image(
        ...
        if first_image.section_type in ["cond_vae_image", "cond_joint_image"]:
            ...
            for cond_images in batch_cond_images:
                ...
                for cond_image in cond_images:
                    vae_image = cond_image.vae_image if cond_image.section_type == "cond_joint_image" else cond_image
        ...
        if first_image.section_type in ["cond_vit_image", "cond_joint_image"]:
            ...
            for cond_images in batch_cond_images:
                cond_vit_image_list = []
                for cond_image in cond_images:
                    vit_image = cond_image.vit_image if cond_image.section_type == "cond_joint_image" else cond_image
                    cond_vit_image_list.append(vit_image)
                # Here we force convert the tensor to dtype
                cond_vit_images.append(torch.stack(cond_vit_image_list, dim=0).to(dtype=self.dtype))
    ...
    def _prepare_vit_image_kwargs(batch_cond_images, cfg_factor):
        ...
        for cond_images in batch_cond_images:
            cond_vit_image_kwargs["spatial_shapes"].append(
                torch.stack(
                    [cond_image.vit_image.vision_encoder_kwargs["spatial_shapes"] for cond_image in cond_images]
                )
            )
            cond_vit_image_kwargs["attention_mask"].append(
                torch.stack(
                    [cond_image.vit_image.vision_encoder_kwargs["pixel_attention_mask"] for cond_image in cond_images]
                )
            )
        if cfg_factor > 1:
                ...
                for item in content:
                    if item["type"] == "text":
                        ...
                        if all(key not in item for key in ["image", "url", "path", "base64"]):
                            continue
                        assert cond_images is not None and image_idx < len(
                            cond_images
        ...
        #   -- 2.2 Prompt, image, cot text, system prompt
            ...
            if batch_cond_images is None:
                batch_cond_images = (
                    [
                        self.image_processor.build_cond_images(
                            image_list=image_list, infer_align_image_size=infer_align_image_size
                        )
                        for image_list in batch_image_list
                    ]
                    if batch_image_list is not None
                    else None
                )
    ...
    def prepare_model_inputs(
        ...
        **kwargs,
        ...
        cfg_factor = {"gen_text": 1, "gen_image": 2, "train_image": 1}
        if self.config.cfg_distilled:
            cfg_factor["gen_image"] = 1

        bot_task = kwargs.pop("bot_task", "auto")
        use_cached_cond_inputs = kwargs.pop("use_cached_cond_inputs", False)
        cached_cond_vae_images = kwargs.pop("cond_vae_images", None)
        cached_cond_timesteps = kwargs.pop("cond_timesteps", None)
        cached_cond_vit_images = kwargs.pop("cond_vit_images", None)
        cached_cond_vit_image_kwargs = kwargs.pop("cond_vit_image_kwargs", None)
        ...
        if out is None:
            out = self.preprocess_inputs(
                prompt=prompt,
                image=image,
                mode="gen_image" if mode in ["gen_image", "train_image"] else "gen_text",
        ...
        if use_cached_cond_inputs:
            cond_vae_images = cached_cond_vae_images
            cond_timesteps = cached_cond_timesteps
            cond_vit_images = cached_cond_vit_images
            cond_vit_image_kwargs = cached_cond_vit_image_kwargs
        else:
        ...
        # 6. Build kv cache
        if mode == "gen_image" or mode == "train_image":
        ...
        batch_position_ids = torch.arange(0, output.tokens.shape[1], dtype=torch.long, device=device)[None].expand(
    ...
    def _prepare_attention_mask_for_generation(
        ...
        batch_full_attn_slices = [
            self.image_processor.prepare_full_attn_slices(tokenizer_output, i) for i in range(bsz)
        ]
        # if len(batch_full_attn_slices[0]) == 0:
        #    return None

        attention_mask = (
            torch.ones(seq_len, seq_len, dtype=torch.bool, device=inputs_tensor.device)
            .tril(diagonal=0)
            .repeat(bsz, 1, 1)
        )
        for i in range(bsz):
    ...
    def _update_model_kwargs_for_generation(
        ...
        if "tokenizer_output" in model_kwargs:
            ...
            if mode == "gen_text":
                ...
                timestep_position_ids = index[torch.arange(bsz), model_kwargs["timesteps_index"][:, -1]].unsqueeze(-1)
                pos_cat_list = [
                    timestep_position_ids,
                ]
                if self.config.cfg_distilled:
                    guidance_position_ids = index[torch.arange(bsz), model_kwargs["guidance_index"][:, -1]].unsqueeze(
                        -1
                    )
        ...
        if mode == "gen_text":
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                if stage_transitions is not None:
                    ...
                    logits_processor.append(self._StageTransitionLogitsProcessor(stage_transitions, input_ids.shape[0]))
    ...
    def generate_image(
        ...
        **kwargs,
        ...
        pg_state_dict = kwargs.pop("pg_state_dict", None)
        if bot_task in ["think", "recaption", "think_recaption"]:
            ...
            if first_bot_task == "think" and "recaption" in bot_task:
                stage_transitions.append((tkw.end_of_think_token_id, [tkw.convert_tokens_to_ids(tkw.recaption_token)]))
        ...
        if pg_state_dict is not None:
            self.set_pg_state_dict(pg_state_dict)
        batch_cond_images_cache = model_inputs["batch_cond_images"]
        outputs = self.generate(**model_inputs, **kwargs)
        if pg_state_dict is not None:
            self.clear_pg_state_dict()
