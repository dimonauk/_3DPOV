/**
 * app/api/splatter/scout/route.ts — Scout for the website.
 *
 * Two paths:
...
import { benchInfo, bench } from "lib/splatter/bench-client";
import { scoutFromMetadata } from "lib/splatter/scout-web";
  ...
  if (body.source_path) {
    ...
    if (info.reachable) {
      try {
        ...
        console.warn("[splatter/scout] bench unreachable:", err);
      }
    }
  }
...
async function tryLlmEnrich(report: ScoutReport): Promise<ScoutReport> {
  try {
    ...
    console.warn("[splatter/scout] LLM enrich failed:", err);
    return report;
  }
}
