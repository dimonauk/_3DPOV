/**
 * app/api/atelier/waveguide-forge/generate-hdri/route.ts —
 * The waveguide-forge chamber's HDRI-generation route.
 *
 ...
 * resulting equirect image URL so the chamber can drape it over the
 * scene as environment lighting + background.
 *
 * Why `flux-equirect-lora-v3`: per `generate-comfyui.PURPOSE.md` it is
 * the "best 360 quality on the bench" — the right pick for hero-grade
 * chamber backdrops. Same workflow the cube-composer route uses.
 *
 * # Body
 * JSON `{ prompt: string }`.
 *
 * # Timing
 * The bench takes ~30-60s for a 4K equirect from Flux + LoRA. We bump
 * the Vercel function timeout to 300s to keep a margin for cold-start
 * + queue.
 */
