...
/**
 * Per-level page. One dynamic segment carries all twelve levels — eight
 * solo (module, trail, loop, witness, sovereignty, curriculum, perch,
 * bezel) and four braided (the-2-braid, the-3-beat-weave, the-5-beat,
 * the-full-weave). The level is looked up from lib/play.ts; the route
 ...
 * only) or the level-specific stub component. Aura register throughout.
 */
...
export async function generateMetadata({
  ...
  return {
    title: `${entry.name} — Play, ${phaseLabel} ${entry.level}`,
...
function SceneMount({ level }: { level: PlayLevel }) {
  // Trail is the only live/preview-playable scene today. Every other
  // level mounts a stub component that names the interaction shape
  // and the integration point — the visual contract the level
  // commits to before the gameplay lands.
