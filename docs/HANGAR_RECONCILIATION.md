...
| B4 | `articles/the-fleet-four-airframes` | User memory says "Five airframes when the fleet comes up" | **Reconciliation point — see Class C #C1.** The site says four; the canon says five. The fifth is the LED-modified payload variant treated as a separate airframe at the planning level. Edit title + opening paragraph + the closing "what this fleet is for" paragraph. |
...
|---|---|---|---|---|
| C1 | **Fleet count** | "Four airframes" (`articles/the-fleet-four-airframes.tsx`, title + line 10 + line 13) | User explicitly: "Five airframes when the fleet comes up." | **Rewrite to five.** The fifth is the LED-modified payload variant which currently exists in the article as "not a fifth drone" (line 253). The user wants it counted. Update title slug (`the-fleet-five-airframes`?), opening, count, closing. Register: matter-of-fact. The LED airframes are a different mission shape; honest to count them. |
