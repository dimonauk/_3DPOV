...
/**
 * Parsed VRM Animation. The canonical pixiv/three-vrm-animation
 * port — vendored here so we don't have to depend on the npm package
 * (which would pin specific three.js versions and lag behind core).
 *
 * Built from a .vrma file by VRMAnimationLoaderPlugin. Bake into a
 * playable Three.js AnimationClip via `createAnimationClip(vrm)`.
 */
