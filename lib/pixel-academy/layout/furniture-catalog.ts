...
const log = createLogger("play:pixel-academy:furniture-catalog");
...
export function buildDynamicCatalog(assets: LoadedAssetData): boolean {
  ...
  const allEntries = assets.catalog.map((asset) => {
    ...
    if (!sprite) {
      log.warn("no sprite data for asset", { assetId: asset.id });
      return null
    }
    return {
      type: asset.id,
  ...
  log.info("built dynamic catalog", {
    totalAssets: allEntries.length,
    visibleAssets: visibleEntries.length,
    rotationGroups: rotGroupCount,
    statePairs: stateGroups.size / 2,
  });
  return true
}
